...
def safe_add_column(df, name, column_expr):
    try:
        return df.withColumn(name, column_expr)
    except AnalysisException as e:
        print(f"Error adding column {name}: {str(e)}")
        return df
...
for name, data_type in flat_schema:
    if isinstance(data_type, ArrayType) and isinstance(data_type.elementType, StructType):
        ...
        df_parsed = safe_add_column(df_parsed, f"{name}_exploded", explode(col(f"parsed_json.{name}")))
        for subfield in data_type.elementType.fields:
            df_parsed = safe_add_column(df_parsed, f"{name}_{subfield.name}", col(f"{name}_exploded.{subfield.name}"))
        df_parsed = df_parsed.drop(f"{name}_exploded")
    elif isinstance(data_type, ArrayType):
        # For simple arrays, keep them as a single column
        df_parsed = safe_add_column(df_parsed, name, col(f"parsed_json.{name}"))
    else:
        # For simple types, create a column directly
        df_parsed = safe_add_column(df_parsed, name, col(f"parsed_json.{name}"))
...
# If you want to see the actual content of the 'parsed_json' column:
print("\nSample data from parsed_json column:")
